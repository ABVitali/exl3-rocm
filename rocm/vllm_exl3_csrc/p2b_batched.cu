...
#if defined(EXL3_ROCM)
// ROCm: the mma-based vendor GEMV kernel is compiled out of the header, so only its
// fragment types and decode helpers come in. The m > 1 worklist path that called it as a
// device function is not available; p2b_gemv_batched supports K=2 MCG with m == 1.
...
__device__ __forceinline__ void p2b_run_gemv_tile_2(
    ...
    auto ld_b = [&](int i) -> uint32_t {
        return EXL3_LDCS(bp + (size_t)i * slice_stride);
    };
    uint32_t pf[PF];
    #pragma unroll
    for (int d = 0; d < PF; ++d)
...
#if defined(EXL3_ROCM)
    // FMA form of the M == 1 tensor-core tile; see run_gemv_tile in p2b_moe.cu.
    ...
    for (int ib = 0; ib < myn; ib += PF) {
        ...
        for (int d = 0; d < PF; ++d) {
            ...
            const float2 a0 = __half22float2(A2[a_col]);
            const float2 a2 = __half22float2(A2[a_col + 4]);
            #pragma unroll
            for (int t = 0; t < WNT; ++t) {
                const int base = (t & 1) << 4;
                const uint32_t bwv = EXL3_SHFL(w, base + x_src_b);
                const uint32_t awv = EXL3_SHFL(w, base + x_src_a);
                FragB f0, f1;
...
#if defined(EXL3_ROCM)
    else TORCH_CHECK(false, "ROCm port: p2b_gemv_batched supports K=2 MCG with m == 1 only (use p2b_fused_moe)");
#else
    else if (bits == 2 && mcg)
