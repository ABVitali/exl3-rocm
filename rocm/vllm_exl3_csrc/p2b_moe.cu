    ...
    float (*sh_red)[1][32])
{
    ...
    auto ld_b = [&] (int i, int l) -> uint32_t {
        if constexpr (bits == 3)
            return lane < 24 ? EXL3_LDCS(bp + (size_t) i * slice_stride + l * LSTRIDE) : 0;
        else
            return EXL3_LDCS(bp + (size_t) i * slice_stride + l * LSTRIDE);
    };

    // Two trellis words per lane -> eight decoded weights in m16n8k16 B-fragment order:
    // f0/f1 = n-subtiles 0..7 / 8..15 of tile t; lane l holds column n = l/4 at
    // k = (l%4)*2 + {0, 1} (elem 0) and + {8, 9} (elem 1).
    auto decode_tile = [&] (const uint32_t* bw, int t, FragB& f0, FragB& f1) {
        if constexpr (bits == 4) {
            uint32_t aw = EXL3_SHFL(bw[t], (lane + 31) & 31);
            exl3_gemv_ns::dq8_regs_4bits<cb>(aw, bw[t], f0, f1);
        } else if constexpr (bits == 2) {
            const uint32_t w = bw[t >> 1];
            const int base = (t & 1) << 4;
            uint32_t bwv = EXL3_SHFL(w, base + x_src_b);
            uint32_t awv = EXL3_SHFL(w, base + x_src_a);
            exl3_gemv_ns::dq8_regs_2bits<cb>(awv, bwv, lane << 3, f0, f1);
        } else {
            uint32_t awv = EXL3_SHFL(bw[t], x_src_a);
            uint32_t bwv = EXL3_SHFL(bw[t], x_src_b);
...
#ifdef EXL3_ROCM
    // ROCm: no tensor-core mma. With M == 1 the 16x8 product per n-subtile is one dot
    // product per output column, so every lane multiplies its four B values by the four
    // matching A values and accumulates in fp32; the four lanes sharing a column reduce
    // with two xor shuffles at the end. Lane->(k, n) mapping verified against the PTX
    // fragment spec and reconstruct_kernel's swizzle in tools/mma_layout_check.cpp.
    (void) hzero; (void) FOLD;
    float acc[WNT][2] = {};
    ...
    for (int ib = 0; ib < myn; ib += PF) {
        ...
        for (int d = 0; d < PF; ++d) {
            ...
            // every lane reads A at its own k slots (the CUDA path only needs lanes 0-3 to)
            const size_t a_col = (size_t) (ks0 + i) * 8 + (lane & 3);
            const float2 a0 = __half22float2(A2[a_row0 + a_col]);
            const float2 a2 = __half22float2(A2[a_row0 + a_col + 4]);

            #pragma unroll
            for (int t = 0; t < WNT; ++t) {
                FragB f0, f1;
                decode_tile(bw, t, f0, f1);
                const float2 b00 = __half22float2(f0[0]);
                const float2 b01 = __half22float2(f0[1]);
                const float2 b10 = __half22float2(f1[0]);
                const float2 b11 = __half22float2(f1[1]);
                acc[t][0] += a0.x * b00.x + a0.y * b00.y + a2.x * b01.x + a2.y * b01.y;
                acc[t][1] += a0.x * b10.x + a0.y * b10.y + a2.x * b11.x + a2.y * b11.y;
            }
        }
    }

    // Reduce over the 4 lanes that share n = lane / 4
    #pragma unroll
    for (int t = 0; t < WNT; ++t)
        #pragma unroll
        for (int f = 0; f < 2; ++f) {
    ...
    if ((lane & 3) == 0) {
        ...
        for (int t = 0; t < WNT; ++t)
            #pragma unroll
            for (int f = 0; f < 2; ++f)
                sh_red[warp][0][t * 16 + f * 8 + n] = acc[t][f];
    }
#else
