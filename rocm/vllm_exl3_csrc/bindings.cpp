...
at::Tensor exl3_gemm(const at::Tensor& x, const at::Tensor& trellis,
    ...
    auto x2 = x.contiguous().view({x.numel() / x.size(-1), x.size(-1)});
...
#endif
    // ROCm: no tensor-core GEMV; this is the CPU-decoded reference path (tests + rare fallback)
    auto w = dequant_trellis(trellis, suh, svh, bits, mcg);
    return at::mm(x2.to(at::kFloat), w.to(at::kFloat)).to(x.scalar_type());
}
...
#if !defined(EXL3_ROCM)  // tensor-core fat GEMM: CUDA only for now
