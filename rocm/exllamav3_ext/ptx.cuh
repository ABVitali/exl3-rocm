...
#ifndef EXL3_ROCM  // ---- inline-PTX section: mma, barriers, cp.async, ldmatrix, cache-hinted ld/st ----
...
__device__ inline uint32_t mul_hi_u32(uint32_t x, uint32_t y) { return __umulhi(x, y); }
#endif  // EXL3_ROCM
...
{
    uint64_t value = (static_cast<uint64_t>(hi) << 32) | static_cast<uint64_t>(lo);
    return static_cast<uint32_t>((value >> offset) & ((1ull << length) - 1ull));
}
// shf.r.wrap.b32 dst, lo, hi, imm : dst = (hi:lo) >> (imm & 31)
#define FSHF_IMM(dst, lo, hi, imm) (dst) = (uint32_t)(((((uint64_t)(hi)) << 32) | (uint64_t)(lo)) >> ((imm) & 31))
// bfe.u32 dst, src, imm, 16 : 16-bit field at bit offset imm
