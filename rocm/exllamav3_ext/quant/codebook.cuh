...
__device__ inline half2 decode_mul1_product_2(uint32_t x0, uint32_t x1)
{
    ...
    uint32_t sum0 = EXL3_DP4A(x0, 0x01010101u, acc);
    uint32_t sum1 = EXL3_DP4A(x1, 0x01010101u, acc);
...
{
    EXL3_LOP3_6A(x0, 0x8fff8fff, 0x3b603b60);
    EXL3_LOP3_6A(x1, 0x8fff8fff, 0x3b603b60);
    half2_uint32 xu0(x0);
    half2_uint32 xu1(x1);
...
__device__ inline half decode_3inst(uint32_t x)
{
    if constexpr (cb == 0)
    {
        x *= 89226354u;
        x += 64248484u;
        EXL3_LOP3_6A(x, 0x8fff8fff, 0x3b603b60);
        half2_uint32 xu(x);
    ...
    if constexpr (cb == 2)
    {
        ...
        uint32_t sum = EXL3_DP4A(x, 0x01010101u, acc);
...
//    x = ((x * 1664525u) >> 32) + x;
//    int32_t c = (int32_t) EXL3_DP4A((uint32_t) x, 0x01010101u, 0xFFFFFE02u);
