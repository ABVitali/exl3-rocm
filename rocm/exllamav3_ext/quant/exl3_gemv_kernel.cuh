...
#endif  // EXL3_ROCM: no tensor-core mma; p2b_* use the FMA tile instead
...
__device__ __forceinline__ half2 decode_pair_cb2_dp4a_(uint32_t x0, uint32_t x1)
{
    x0 *= 0x83DCD12Du;
    x1 *= 0x83DCD12Du;
    uint32_t sum0 = EXL3_DP4A(x0, 0x01010101u, 0x6400u);
    uint32_t sum1 = EXL3_DP4A(x1, 0x01010101u, 0x6400u);
...
#ifndef EXL3_ROCM  // cooperative GEMV kernel (mma + m>1 fragments): CUDA only for now
