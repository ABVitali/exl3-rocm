...
//    __shared__ half tile[16 * 8 * 16];
    ...
    half2 n0 = EXL3_SHFL_DOWN(frag[0][0], 4);
    half2 n1 = EXL3_SHFL_DOWN(frag[0][1], 4);
    half2 n2 = EXL3_SHFL_DOWN(frag[1][0], 4);
    half2 n3 = EXL3_SHFL_DOWN(frag[1][1], 4);
...
)
{
    ...
    auto kfn_ = reconstruct_kernel_instances[cbi];

    kfn_<<<gridDim, blockDim, 0, stream>>>
    (
...
)
{
    ...
    auto kfn_ = reconstruct_had_kernel_instances[cbi];

    kfn_<<<gridDim, RH_THREADS, 0, stream>>>
    (
        (half*) unpacked.data_ptr(),
        (const uint16_t*) packed.data_ptr(),
