...
__device__ inline void shuffle_had_f4x32(float& h0, float& h1, float& h2, float& h3, const int lane_id)
{
    ...
    for (int i = 1; i < 32; i <<= 1)
    {
        ...
        uint64_t ph01 = (uint64_t) EXL3_SHFL_XOR((unsigned long long) h01, i);
        uint64_t ph23 = (uint64_t) EXL3_SHFL_XOR((unsigned long long) h23, i);
...
__device__ inline void shuffle_had_f2x32(float& v, float& w, const int lane_id)
{
    ...
    for (int i = 1; i < 32; i <<= 1)
    {
        ...
        uint64_t pvw = (uint64_t) EXL3_SHFL_XOR((unsigned long long) vw, i);
...
__device__ inline half2 shuffle_had_h2x32(half2 v, int lane_id)
{
    for (int i = 1; i < 32; i <<= 1)
    {
        uint32_t vu_ = *reinterpret_cast<uint32_t*>(&v);
        vu_ = EXL3_SHFL_XOR(vu_, i);
        half2 pv = *reinterpret_cast<half2*>(&vu_);
        uint32_t* vi = reinterpret_cast<uint32_t*>(&v);
        int32_t sfm = -static_cast<int16_t>(lane_id & i) >> 31;
        *vi ^= (sfm & 0x80008000);
        v = __hadd2(v, pv);
...
)
{
    ...
    switch (act_function)
    {
        ...
        case ACT_RELU2_NOGATE:
            vg.x = EXL3_HMAX2(vu.x, __float2half2_rn(0.0f));
            vg.y = EXL3_HMAX2(vu.y, __float2half2_rn(0.0f));
            break;
    ...
    if (act_limit != 0.0f)
    {
        vu.x = EXL3_HMAX2(vu.x, __float2half2_rn(-act_limit));
        vu.y = EXL3_HMAX2(vu.y, __float2half2_rn(-act_limit));
        vu.x = EXL3_HMIN2(vu.x, __float2half2_rn(act_limit));
        vu.y = EXL3_HMIN2(vu.y, __float2half2_rn(act_limit));
        vg.x = EXL3_HMIN2(vg.x, __float2half2_rn(act_limit));
        vg.y = EXL3_HMIN2(vg.y, __float2half2_rn(act_limit));
    }
